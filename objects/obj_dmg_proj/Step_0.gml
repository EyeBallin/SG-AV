if (global.ctrlGameState.currGameState == gameStateEnum.stateActiveGame) {
	...
	if (
		projOwner.entityType == entityTypes.ePlayer || 
		projOwner.entityType == entityTypes.eAlly) {
			collCheck = obj_enemy;
	} else if (
		projOwner.entityType == entityTypes.eEnemyBoss || 
		projOwner.entityType == entityTypes.eEnemyNormal || 
		projOwner.entityType == entityTypes.eEnemyMinion) {
			collCheck = obj_sg_av;
	}
