if (global.ctrlGameState.currGameState == gameStateEnum.stateActiveGame) {
	...
	if (auraTimer == 0) {
	  if (
			auraOwner.entityType == entityTypes.ePlayer || 
			auraOwner.entityType == entityTypes.eAlly) {
				collCheck = obj_enemy;
		} else if (
			auraOwner.entityType == entityTypes.eEnemyBoss || 
			auraOwner.entityType == entityTypes.eEnemyNormal || 
			auraOwner.entityType == entityTypes.eEnemyMinion) {
				collCheck = obj_sg_av;
		}
	}
	auraCodeStep(self);
